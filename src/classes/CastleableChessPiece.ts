...
export abstract class CastleableChessPiece extends ChessPiece {
            ...
            if (possiblePosition === null) {
              ...
              if (
                ...
                [PieceType.King, PieceType.Rook]
                  ...
                  .includes(possiblePosition.type) &&
                possiblePosition.atInitialPosition()
              ) {
  ...
  canCastle = (boardState: BoardState, castlePath: string[]) => {
    ...
    const availablePieces = currentBoard
      .flatMap((piece) => piece)
      .filter((piece) => piece !== null && piece.color !== this.color);
    const fakeBoard = cloneBoardState(currentBoard);

    castlePath.forEach((positionId) => {
      const [px, py] = getCoordinateFromBoardId(positionId);
      const placeholderKing = new PlaceholderChessPiece(
        this.color,
        [px, py],
        PieceType.King
      );
